    ...
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
        <div>
          ...
          <p style={{ color:'#94A3B8', fontSize:13, marginTop:4 }}>{filtered.length} alumnos · haz clic en un alumno para ver su perfil</p>
        </div>
         ...
         filtered.length===0?<div style={{ padding:'3rem', textAlign:'center', color:'#94A3B8', fontSize:13 }}>Sin alumnos.</div>:(
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            <thead><tr style={{ background:'#F8FAFC', borderBottom:'1px solid #E2E8F0' }}>
              {['Alumno','Edad','Nivel','Estado','Tutor','Teléfono',''].map(h=>(
            ...
            <tbody>{filtered.map((s:any)=>{
              ...
              return (
                <tr key={s.id} style={{ borderBottom:'1px solid #F8FAFC', cursor:'pointer' }} onClick={()=>window.location.href=`/admin/students/${s.id}`}>
                  ...
                  <td style={{ padding:'12px 16px' }} onClick={e=>e.stopPropagation()}>
                    {s.parents?.phone&&(
                      <a href={`https://wa.me/${s.parents.phone.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer" style={{ width:30, height:30, borderRadius:8, background:'#ECFDF5', display:'inline-flex', alignItems:'center', justifyContent:'center', textDecoration:'none', fontSize:14 }}>💬</a>
                    )}
                  </td>
                </tr>
              )
